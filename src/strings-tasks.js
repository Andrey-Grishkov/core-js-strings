...
function sumOfCodes(str) {
  if (!str) return 0;
  return str.split('').reduce((acc, symbol) => acc + symbol.charCodeAt(), 0);
}
...
function countVowels(str) {
  ...
  return str.split('').reduce((acc, symbol) => {
...
function isPalindrome(str) {
  const changeStr = str
    ...
    .toLowerCase()
    .replace(/[^\w\s]/g, '');
  return changeStr === changeStr.split('').reverse().join('');
}
...
function findLongestWord(sentence) {
  const lengthWordsMass = sentence.split(' ').map((word) => word.length);
  const maxWordLength = lengthWordsMass.reduce(
    (acc, number) => (number > acc ? number : acc),
    0
  );
  const index = lengthWordsMass.indexOf(maxWordLength);
...
function encodeToRot13(str) {
  return str
    ...
    .map((symbol) => {
      ...
      const translateCharCode =
        charCode + (symbol.toLowerCase() < 'n' ? 13 : -13);
      switch (symbol) {
        case ' ':
          return ' ';
        case '?':
      ...
      return String.fromCharCode(translateCharCode);
    })
    .join('');
}
